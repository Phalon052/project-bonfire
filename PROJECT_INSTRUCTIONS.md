...
- **`tools/pricing.py`**: the plate price. **Price = ((filament g × 0.013) + (2 × print hours)) + 15%**, per plate, from the slicer's figures. `price_3mf("<file>.gcode.3mf")` prices every plate of a sliced file plus the file total; `plate_price(grams, hours)` for one plate; `python pricing.py <grams> <hours>` from a terminal. The slice report includes it automatically. Every time a .3mf is set up to print, report the price of each plate and the total for that .3mf (`04_bambu_basics.md` §4).
