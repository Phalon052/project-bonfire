...
| **Material-aware tolerances** | • Per-side clearances by filament and fit type (Overture PLA measured; other filaments set when first used)<br>• Measured values replace the defaults once recorded; a PLA tolerance test set is designed | Done |
...
| **3D Print Toolbox export gate** | • Blender's 3D Print Toolbox add-on checks every part before export: closed solid, self-intersections, wall thickness, overhangs<br>• A failed check blocks the STL export; a second check runs on the exported file<br>• Measured minimum wall thickness fills the printable-minimums rule | Planned |
...
| **Fill-outs** | • The `0n_<title>.md` rule files have sections marked **[Fill out]**. These are for me to set up with my own preferences and test results<br>• My current focus is building key end-to-end functionality and optimizing steps that give good results but are slow or use too many tokens<br>• Once testing sets a baseline, I'll have the information I need and these sections will be removed<br>• This will also allow a single set of test prints whose results feed a script that auto-configures defaults for your machine | Planned |
